...
#Borramos el data frame original (mydf)
rm(mydf)
```
...
#Usamos la función glimpse que proporciona una vista rápida de la estructura de los datos
glimpse(cran)
...
```{r}
cran %>%
  filter (r_os == "linux-gnu", size > 100500)
...
```{r}
#preterminado el  orden ascendente
cran2 %>%
  arrange(ip_id)
...
#Para deshacer con la función ungroup
ungroup(by_package)
...
by_package %>%
  summarise (valor_medio = mean(size))
